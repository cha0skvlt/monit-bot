...
3. Соберите и запустите контейнер:
   ...
   ```
   Файл `status.json` должен существовать перед запуском `docker compose up`. Контейнер автоматически монтирует файлы `sites.txt`, `status.json` и каталог `logs`. Бот перезапускается при сбое благодаря опции `restart: always`.
...
- `/start` – краткая информация и меню команд.
